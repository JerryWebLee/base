...
class mainCanvasRenderer extends CanvansRenderBase {
    ...
    async run() {
        ...
        let body = await this.loadFBX('../resource/models/ALL(3).FBX');
    ...
    animate() {
        ...
        this.controls.target.set(0, this.controls.target.y, 0);


       
        // if (this.currentShowPoint != null){
        //     console.log(this.currentShowPoint.obj);
        //     if (this.currentShowPoint.obj.children[0].children[0].scale.x > 0.001){
        //         this.currentShowPoint.obj.children[0].children[0].scale.x *= 0.9;
        //         this.currentShowPoint.obj.children[0].children[0].scale.y *= 0.9;
        //         this.currentShowPoint.obj.children[0].children[0].scale.z *= 0.9;
        //     }
        // }
    ...
    bodyEffect() {
        ...
        this.hideArr = [];
        this.MuscleManager = new MuscleManager(muscleArr);
    ...
    onMouseMove(e) {
        ...
        if (!hit_skin){
            ...
            let boxes = raycaster.intersectObjects(this.boxArr).object;
            ...
            if (obj && obj.material && obj.material.name == "Muscular_System"){ // 鼠标移到了肌肉上
                if (obj.material.opacity != 1){ // 目前是透明的

                }
                else{
                    // this.highLightToogle.toogle(obj.name);
                    let pos = obj.name.lastIndexOf('_');
                    let index = obj.name.substring(0, pos);
                    // console.log(index);
                    this.MuscleManager.setHighlight(index);
                }                
                ...
                this.MuscleManager.unHighlight();
            }
...
class showPoint_base {
    ...
    onMouseOver(e) {
        ...
        for (var i=0; i<this.adjMuscleIndex.length; i++){
            this.body.MuscleManager.setTransparent(this.adjMuscleIndex[i]);
        }        
...
class MuscleManager{
    constructor(muscleArr){
        ...
        this.oldMat = [];
        this.transparentIndexArr = [];
        this.highlightIndex = '';
    }
    setTransparent(indexArr){
        if (this.muscleArr[index].material == this.highLightMat){
            this.muscleArr[index].material = this.oldMat;
        }
        if (!this.transparentMat){
            let mat = this.muscleArr[index].material.clone();
            this.oldMat = this.muscleArr[index].material.clone();
            console.log(mat);
            mat.transparent = true;
            mat.opacity = 0.7;
            this.transparentMat = mat;
            console.log(this.transparentMat);
        }
        this.transparentIndexArr.push(index);
        this.muscleArr[index].material = this.transparentMat;
    }
    unTransparentAll(){
        for (var i=0; i<this.transparentIndexArr.length; i++){
            let index = this.transparentIndexArr.shift();
            this.muscleArr[index].material = this.oldMat;
        }
    }
    setHighlight(index){
        if (this.muscleArr[index].material == this.transparentMat){
            this.muscleArr[index].material = this.oldMat;
        }
        this.unHighlight();
        if (!this.highLightMat){
            ...
            console.log(mat)
            mat.color = new THREE.Color(0xff5555);
            this.highLightMat = mat;
            // mat = this.muscleArr[index].material[1].clone();
            // mat.color = new THREE.Color(0xff5555);
            // this.highLightMat.push(mat);
            // this.highLightMat = this.muscleArr[index].material.clone();
            // this.oldMat = this.muscleArr[index].material.clone();
            // this.highLightMat.color = new THREE.Color(0xff5555);
            console.log(this.highLightMat)
        }
        this.highlightIndex = index;
        // console.log(this.highLightMat);
        this.muscleArr[index].material = this.highLightMat;
    }
    unHighlight(){
        if (this.highlightIndex){
            this.muscleArr[this.highlightIndex].material = this.oldMat;
        }
    }
}
